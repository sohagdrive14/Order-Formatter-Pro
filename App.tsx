...
const App: React.FC = () => {
  ...
  const formatForCopy = (block: OrderBlock) => {
    let text = `ORDER ID: ${block.orderId}\nNAME: ${block.name}\nCONTACT: ${block.contact}\nCOD BILL: ${block.codBill}\nADDRESS: ${block.address}`;
    if (block.tracking) text += `\nTRACKING: ${block.tracking}`;
    return text;
  };
  ...
  const exportPDF = () => {
    ...
    state.result.forEach((order, index) => {
      ...
      doc.rect(14, yPos, pageWidth - 28, 65);
      ...
      doc.text(`ORDER ID: ${order.orderId}`, 18, yPos + 16);
      doc.text(`NAME: ${order.name}`, 18, yPos + 24);
      doc.text(`CONTACT: ${order.contact}`, 18, yPos + 32);
      doc.text(`COD BILL: ${order.codBill}`, 18, yPos + 40);
      
      let currentY = yPos + 48;
      if (order.tracking) {
        doc.text(`TRACKING: ${order.tracking}`, 18, currentY);
        currentY += 8;
      }
      ...
      yPos += 55; // Reduced height since note is removed
    });
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl w-full space-y-8">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-start">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-0.5">Order ID</p>
                            <p className="text-base font-black text-slate-900 truncate">{order.orderId}</p>
                          </div>
                        </div>
                        ...
                        {order.tracking && (
                          <div className="flex items-start">
                            <div className="bg-cyan-50 p-2.5 rounded-xl mr-4 group-hover:bg-cyan-100 transition-colors">
                              <Search className="h-5 w-5 text-cyan-600" />
                            </div>
                            <div>
                              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-0.5">Tracking</p>
                              <p className="text-base font-bold text-slate-800">{order.tracking}</p>
                            </div>
                          </div>
                        )}
          ...
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl animate-modal overflow-hidden">
              ...
              <div className="p-8 space-y-5">
                <div>
                  <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Order ID</label>
                  <input 
                    type="text" 
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Tracking ID</label>
                    <input 
                      type="text" 
        ...
        {/* Loading Overlay */}
        {state.isProcessing && (
          <div className="fixed inset-0 bg-white/95 backdrop-blur-xl z-[60] flex flex-col items-center justify-center animate-in fade-in duration-700">
            <div className="relative mb-12">
              {/* Outer Pulse Orbs */}
              <div className="absolute inset-0 bg-indigo-400 rounded-full animate-smooth-pulse blur-3xl"></div>
              <div className="absolute inset-0 bg-blue-400 rounded-full animate-smooth-pulse delay-700 blur-3xl"></div>
              
              {/* Main Animated Container */}
              <div className="relative w-40 h-40 bg-white rounded-[2.5rem] shadow-2xl flex items-center justify-center border border-slate-100 overflow-hidden">
                {/* Scanning Line */}
                <div className="absolute left-0 right-0 h-1 bg-gradient-to-r from-transparent via-indigo-500 to-transparent animate-scan"></div>
                
                {/* Rotating Inner Ring */}
                <div className="absolute inset-4 border-2 border-dashed border-indigo-100 rounded-[2rem] animate-[spin_10s_linear_infinite]"></div>
                
                {/* Central Icon Stack */}
                <div className="relative flex flex-col items-center">
                  <div className="relative">
                    <Cpu className="h-14 w-14 text-indigo-600 animate-pulse" />
            ...
            <div className="text-center space-y-3 px-6">
              ...
              <p className="text-slate-500 font-bold text-sm uppercase tracking-[0.2em] animate-pulse">
                Gemini AI is reading your orders
              </p>
            </div>
          </div>
        )}
